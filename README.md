# **Penting Baca Dulu Siapa Tau Lupa :)** #
Aplikasi ini dibangun menggunakan server lokal sehingga ketika akan mengakses aplikasi ini, pastikan Anda sudah mengaktifkan server lokal dengan menuliskan pada terminal di dalam folder 'android' berupa ** npx json-server --host (IP Address -> 192.168.48.52) geoease.json' **. Setelah itu pastikan untuk jsonUrl pada file Mapdata.html, Createdata.js, Listdata.js, dan Editdata.js berupa ** const jsonUrl = 'http://(IP Address sama dengan eksekusi mengaktifkan server lokal -> 192.168.48.52):3000/geoease'; **. Alamat IP Address menyesuaikan eksekusi pada emulator virtual Android Studio atau fisik, apabila menggunakan emulator virtual Android Studio maka IP Address-nya ** http://10.0.2.2:3000/geoease **. Jika menggunakan fisik maka IP Address-nya tergantung pada koneksi Wi-Fi yang sama digunakan pada Laptop dan Smartphone.
